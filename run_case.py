...
class RunCase(object):
    ...
    def __init__(self,file_name,sheet_name=None,sheet_index=0):
        ...
        self.logger = Logger("RunCase").get_logger_with_level()
    ...
    def run_case_by_row_no(self,row_no):
        ...
        if run_flag == '否':
            ...
            if request_method == 'get':
                ...
                # 将字符串转换成json对象
                json_param = json.loads(request_param)
                response = self.requestUtil.do_post(url,json_param,'',headers,cookies)
            response_text = response.text.strip()
            self.logger.info("请求结果：%s"%response_text)
    ...
    def data_depend(self,request_param):
        ...
        match = re.findall(r'\$\{.+?\..+?\}', request_param)
        if match is None or match == []:
            return request_param
        else:
            for var_pattern in match:
                # 只考虑匹配到一个的情况
                ...
                # 参数替换
                request_param_final = request_param.replace(var_pattern,result,1)
